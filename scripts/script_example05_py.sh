...
joinWithRetry () {
	...
	do
		...
		if [ $res -ne 0 -a $COUNTER -lt $MAX_RETRY ]; then
			...
			echo "PEER$1 failed to join the channel '$CHANNEL_NAME$i', Retry after 2 seconds"
			sleep 2
			joinWithRetry $1
		else
			COUNTER=0
...
chaincodeInvokeEx05() {
        local channel_num=$1
        local peer=$2
        local ch_name_mycc02=$3
	peer chaincode invoke -o $ORDERER_IP:7050 -C $CHANNEL_NAME$channel_num -n mycc05 -c "{\"Args\":[\"invoke\",\"mycc02\", \"sum\", \"$ch_name_mycc02\"]}" >&log.txt
	res=$?
	cat log.txt
	verifyResult $res "Invoke execution on PEER$peer failed "
...
chaincodeQuery () {
  ...
  local peer=$2
  local res=$3
  local ch_name_mycc02=$4


  echo "channel_num : $channel_num"
  echo "channel_name/channel_num : $CHANNEL_NAME$channel_num"
  echo "ch_name_mycc02 : $ch_name_mycc02"
  echo "===================== Querying on PEER$peer on $CHANNEL_NAME$channel_num/mycc02===================== "
  local rc=1
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME$channel_num -n mycc05 -c "{\"Args\":[\"query\",\"mycc02\", \"sum\", \"$ch_name_mycc02\"]}" >&log.txt
#     peer chaincode query -C $CHANNEL_NAME$channel_num -n mycc05 -c "{\"Args\":[\"query\",\"mycc02\", \"sum\", \"\"]}" >&log.txt
#     peer chaincode query -C $CHANNEL_NAME$channel_num -n mycc05 -c "{\"Args\":[\"query\",\"mycc02\", \"sum\"]}" >&log.txt
  ...
  if test $rc -eq 0 ; then
	echo "===================== Query on PEER$peer from mycc05 on $CHANNEL_NAME$channel_num/mycc02 is successful ===================== "
	echo
  else
...
#Instantiate chaincode on channel 0 
	echo "Instantiating chaincodes ex05 on channel myc0 from peer0 ..."
````````#instantiate cc_ex05 on channel0
	instantiateChaincodeEx05 0 myc mycc05 0
	sleep 20
	echo "Instantiating chaincodes ex02 on channels myc0 and myc1 from peer0 ..."
	instantiateChaincodeEx02 0 myc mycc02 0 
        sleep 20
	instantiateChaincodeEx02 0 myc mycc02 1 
...
#Invoke/Query on all chaincodes on all channels
	...
	chaincodeQuery $ch $PEER 300 myc0
        sleep 20
	chaincodeQuery $ch $PEER 300 myc1
	...
	chaincodeInvokeEx05 $ch $PEER myc0
        sleep 20
	chaincodeInvokeEx05 $ch $PEER myc1
	sleep 20

	ch=0
	PEER=1
	ch_name_mycc02=$CHANNEL_NAME$5
	setGlobals "$PEER"
	chaincodeQuery $ch $PEER 300 myc0
        sleep 20
	chaincodeQuery $ch $PEER 300 myc1
	...
	echo
        echo "===================== All GOOD, End-2-End execution for chaincode example05 completed ===================== "
        echo
        echo "Total execution time $(($(date +%s)-START_TIME)) secs"
        exit 0
`
